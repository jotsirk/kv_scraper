...
class PropertyTickResource {
  ...
  fun findAllActive(): ResponseEntity<List<PropertyTickDTO>> {
    val propertyTicksDTO = propertyTickService.findAllActive().map { it.toDTO() }
  ...
  @PatchMapping
  fun archiveTick(
    @Valid @RequestBody tickId: Long,
  ): ResponseEntity<Void> {
    propertyTickService.archiveTick(tickId)
